...
type Props = {
  ...
  activePlan: { nombre: string; descuento: number };
  onApplyPromo: () => void;
...
export default function CartSummary({
  ...
  return (
    <div className="mt-8 space-y-5 border-t border-black/10 pt-6 sm:pt-7">
      <div className="border-t border-black/10 pt-6 text-sm text-black">
        ...
        <div className="mt-3 flex flex-col gap-3 sm:flex-row">
          ...
          <motion.button
            ...
            whileHover={{ y: -1 }}
            whileTap={{ scale: 0.98 }}
            className="border border-black bg-black px-6 py-3 text-sm font-semibold uppercase tracking-wide text-white transition hover:bg-orange-600 hover:border-orange-600"
          >
            Aplicar
          </motion.button>
          </PermissionGate>
        ...
        {promoMessage ? (
          <p className={`mt-3 text-sm ${promoMessage.type === 'success' ? 'text-green-600' : 'text-orange-600'}`}>
            {promoMessage.text}
          </p>
        ) : null}
          ...
          <div className="mt-4 border border-green-600 bg-green-50 p-4 text-sm">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="border border-orange-600 px-4 py-2 text-sm font-semibold uppercase text-orange-600 transition hover:bg-orange-600 hover:text-white"
              >
                Eliminar
              </motion.button>
              </PermissionGate>
      ...
      {!hasActivePlan && (
        <div className="border-l-4 border-orange-600 bg-neutral-50 p-5 text-sm">
          ...
          <motion.button
            ...
            whileHover={{ y: -1 }}
            whileTap={{ scale: 0.98 }}
            className="mt-5 w-full border border-black bg-black px-5 py-3 text-sm font-semibold uppercase tracking-wide text-white transition hover:border-orange-600 hover:bg-orange-600"
          >Quiero unirme
          </motion.button>
          </PermissionGate>
        </div>
      ...
      <div className="space-y-4 border-t-2 border-black bg-neutral-50 pt-6 text-sm">
        ...
        <div className="flex flex-col gap-3 border-t border-black/10 pt-3">
          ...
          <motion.button
            ...
            whileHover={{ y: -1 }}
            whileTap={{ scale: 0.99 }}
            className="w-full border border-orange-600 bg-orange-600 px-6 py-4 text-sm font-bold uppercase tracking-[0.2em] text-white transition hover:bg-red-600 hover:border-black"
          >
            Pagar ahora
          </motion.button>
          </PermissionGate>
