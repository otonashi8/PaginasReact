...
class FilterManager(
    ...
    private val getEffectivePrice: (Product) -> Double // Función para obtener el precio real
) {
    ...
    // Metadatos Dinámicos
    // Productos que pertenecen a esta sección (base para los filtros)
    val sectionProducts: StateFlow<List<Product>> = allProducts.map { products ->
        products.filter { product ->
            // Para Marketplace: Solo aprobados (o visibles si no tienen status). Para Tienda: Visibles.
            val isStatusMatch = if (product.isClientProduct) {
                val s = product.status ?: com.example.ezzeta.data.model.ProductStatus.APPROVED
                s == com.example.ezzeta.data.model.ProductStatus.APPROVED
            } else {
                product.isVisible
            }
            isStatusMatch && (isClientProductFilter == null || product.isClientProduct == isClientProductFilter)
        }
    }.stateIn(scope, SharingStarted.WhileSubscribed(5000), emptyList())

    val minLimitPrice: StateFlow<Float> = sectionProducts.map { products ->
        products.minOfOrNull { getEffectivePrice(it).toFloat() } ?: 0f
    }.stateIn(scope, SharingStarted.WhileSubscribed(5000), 0f)

    val maxLimitPrice: StateFlow<Float> = sectionProducts.map { products ->
        products.maxOfOrNull { getEffectivePrice(it).toFloat() } ?: 1000f
    }.stateIn(scope, SharingStarted.WhileSubscribed(5000), 1000f)

    val availableSizes: StateFlow<Set<String>> = sectionProducts.map { products ->
        products.flatMap { it.getAvailableSizes() }.toSet()
    }.stateIn(scope, SharingStarted.WhileSubscribed(5000), emptySet())

    val availableCategoryIds: StateFlow<Set<String>> = sectionProducts.map { products ->
        products.flatMap { it.categoryIds }.toSet()
    }.stateIn(scope, SharingStarted.WhileSubscribed(5000), emptySet())

    val availableSubCategories: StateFlow<Set<String>> = combine(sectionProducts, _selectedCategoryId) { products, catId ->
        products.filter { catId == "1" || productHasCategory(it, catId) }
            .flatMap { it.subCategoryIds }
            .toSet()
    }.stateIn(scope, SharingStarted.WhileSubscribed(5000), emptySet())

    private fun productHasCategory(product: Product, categoryId: String): Boolean {
        return product.categoryIds.contains(categoryId) || product.categoryId == categoryId
    }
    ...
    val filteredProducts: StateFlow<List<Product>> = combine(
        ...
        val minL = args[9] as Float
        val maxL = args[10] as Float

        val actualRange = priceR ?: minL..maxL
        ...
        products.filter { product ->
            ...
            val matchesCategory = catId == "1" || productHasCategory(product, catId)
            
            val matchesSub = when (subCat) {
                "Todo" -> true
                else -> product.subCategoryIds.any { it.equals(subCat, ignoreCase = true) } || 
                        product.name.contains(subCat, ignoreCase = true)
            }
            
            val matchesCampaign = campaign == null || product.campaign == campaign
            val matchesPrice = getEffectivePrice(product).toFloat() in actualRange
            ...
            matchesQuery && matchesCategory && matchesSub && matchesCampaign && matchesPrice && matchesSize && matchesStore
    ...
    fun clearFilters() {
        _priceRange.value = null // reseteo filtro danimcos
