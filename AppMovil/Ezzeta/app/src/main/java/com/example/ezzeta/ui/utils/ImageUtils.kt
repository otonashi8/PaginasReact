...
object ImageUtils {
    ...
    fun saveBannerImage(context: Context, uri: Uri, bannerId: String, isMobile: Boolean): String? {
        return try {
            val inputStream: InputStream? = context.contentResolver.openInputStream(uri)
            val originalBitmap = BitmapFactory.decodeStream(inputStream)
            inputStream?.close()
            ...
            // Redimensionar para optimizar (Desktop: max 1920, Mobile: max 1080 de ancho)
            val maxWidth = if (isMobile) 1080 else 1920
            ...
            val directory = File(context.filesDir, "home_banners")
            if (!directory.exists()) directory.mkdirs()

            val type = if (isMobile) "mobile" else "desktop"
            val file = File(directory, "banner_${bannerId}_$type.jpg")
            val outputStream = FileOutputStream(file)
