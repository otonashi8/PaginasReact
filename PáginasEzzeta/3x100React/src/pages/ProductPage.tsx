    ...
    <section className="space-y-12 sm:space-y-14">
      <div className="border-b border-black/10 bg-white px-0 pb-8">
        <div className="flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-[0.25em] text-black/55 sm:text-sm">
          <Link to="/" className="transition-colors duration-200 hover:text-orange-600">Inicio</Link>
          <span className="text-black/35">/</span>
          <Link to="/tienda" className="transition-colors duration-200 hover:text-orange-600">Tienda</Link>
          <span className="text-black/35">/</span>
      ...
      <div className="grid gap-12 xl:gap-16 lg:grid-cols-[1.18fr_0.82fr]">
          ...
          <div className="mt-5 grid grid-cols-3 gap-2.5 sm:gap-3">
            {uploadedImages.map((imageUrl, index) => (
              <button
                ...
                className={`group h-24 overflow-hidden rounded-none border bg-white transition-all duration-250 sm:h-28 ${selectedImageIndex === index ? 'border-orange-600 shadow-[0_0_0_1px_rgba(220,38,38,0.22)]' : 'border-black/10 hover:border-black/35'}`}
              >
                {imageUrl ? (
                  <img
                    src={imageUrl}
        ...
        <div className="border-none shadow-none p-0 bg-transparent border border-black/10 bg-white p-5 shadow-[0_16px_44px_rgba(0,0,0,0.06)] sm:p-7">
          <p className="text-[10px] font-medium uppercase tracking-[0.35em] text-orange-600">{product.category}</p>
          <h2 className="mt-1 text-sm font-medium uppercase tracking-[0.25em] text-black">{product.subcategory}</h2>

          <div className="mt-6 flex items-end gap-3">
            <p className="text-[30px] font-semibold text-orange-600">S/{resolveProductPrice(product).precioFinal.toFixed(2)}</p>
          </div>
                ...
                {allSizeOptions.map((size) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className={`rounded-none border px-3 py-4 text-sm font-medium tracking-[0.08em] transition-all duration-200 ${isSelected ? 'border-orange-600 bg-white text-black shadow-[0_0_0_1px_rgba(220,38,38,0.22)]' : 'border-black/15 bg-white text-black'} ${isAvailable ? 'hover:border-black/45' : 'cursor-not-allowed border-black/10 text-black/30 line-through'}`}
                    >
            ...
            <div>
              ...
              <div className="mt-4 flex items-center gap-4">
                <motion.button
                  ...
                  className="h-11 w-11 border border-black/15 bg-white flex items-center justify-center transition hover:border-orange-600 hover:text-orange-600"
                >
                  <Minus size={16} />
                </motion.button>
                <input
                ...
                <div className="space-y-4 p-5">
                  ...
                  <div className="flex items-center justify-between border-t border-black/10 pt-4">
                    <div>
                      ...
                      <p className="text-base font-semibold text-orange-600">S/{resolveProductPrice(product).precioFinal.toFixed(2)}</p>
                    </div>
                    <PermissionGate permission={PERMISSIONS.salesCreate}>
                    <motion.button
                      type="button"
                      ...
                      className="flex h-12 w-12 items-center justify-center bg-black text-white transition hover:bg-orange-600">
                      <ShoppingBag size={18} />
                    </motion.button>
                    </PermissionGate>
                  </div>
            ...
            >
              <div className="flex items-center justify-between border-b border-black/10 px-8 py-6">
                ...
                <button
                  ...
                  className="flex h-11 w-11 items-center justify-center border border-black/10 transition hover:border-orange-600 hover:text-orange-600"
                >✕
                </button>
              </div>
                ...
                <div className="space-y-6 p-8">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="mt-3 h-12 w-full border border-black/10 bg-white px-4 outline-none transition focus:border-orange-600"
                    >
                      {allSizeOptions.map((size) => (
                        <option
                          key={size}
                  ...
                  <button
                    ...
                    className="mt-2 h-14 w-full bg-black text-sm font-semibold uppercase tracking-[0.2em] text-white transition hover:bg-orange-600"
                  >Agregar al carrito
                  </button>
                  </PermissionGate>
                </div>
