...
const STORAGE_KEY_PRODUCTOS_ELIMINADOS = StorageKeys.PRODUCTOS_ELIMINADOS;
...
const obtenerIdsProductosEliminados = (): Set<number> => {
    const datos = storageManager.get<string>(STORAGE_KEY_PRODUCTOS_ELIMINADOS) as string | null;
    if (!datos) {
        return new Set();
    }

    try {
        const ids = JSON.parse(datos) as unknown;
        return new Set(
            Array.isArray(ids)
...
const guardarIdsProductosEliminados = (ids: Set<number>): void => {
    storageManager.set(STORAGE_KEY_PRODUCTOS_ELIMINADOS, JSON.stringify([...ids]));
};
...
export function obtenerProductos():Producto[]{
    const idsProductosEliminados = obtenerIdsProductosEliminados();
    const productosCatalogo = getProducts()
        .map(mapearProductoCatalogo)
        .filter((producto) => !idsProductosEliminados.has(producto.id))
...
export function eliminarProducto(id:number){
    const idsProductosEliminados = obtenerIdsProductosEliminados();
    idsProductosEliminados.add(id);
    guardarIdsProductosEliminados(idsProductosEliminados);
