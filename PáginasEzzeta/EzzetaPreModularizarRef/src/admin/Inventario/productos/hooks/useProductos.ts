...
export const useProductos = () => {
    ...
    function borrarProducto(id: number) {
        const producto = productos.find((productoActual) => productoActual.id === id);
        const nombreProducto = producto?.nombre || 'este producto';

        if (!window.confirm(`¿Seguro que quieres eliminar "${nombreProducto}"? Esta acción no se puede deshacer.`)) {
            return;
        }
