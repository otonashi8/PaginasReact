...
export const isSistemaContactoAccess = (accessName: string, accessLabel: string, path?: string | null) => {
  const normalizedAccessName = normalizeSistemaValue(accessName);
  const normalizedAccessLabel = normalizeSistemaValue(accessLabel);
  const normalizedPath = normalizeSistemaValue(path ?? '');

  return (
    normalizedAccessName.includes('contacto') ||
    normalizedAccessLabel.includes('contacto') ||
    normalizedPath.includes('contacto')
  );
};
