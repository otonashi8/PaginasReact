...
const isProductInStock = (product: Product) => product.stock !== 0;
...
export const QuickAddModal = ({ product, initialSize, isOpen, onClose }: QuickAddModalProps) => {
  ...
  const variantOptions = useMemo(() => {
    ...
    const variantEntries = familyProducts.filter(isProductInStock).flatMap((item) => {
  ...
  const sizeOptions = useMemo(() => currentProduct?.sizes ?? [], [currentProduct]);
  const availableSizes = useMemo(() => {
    if (!currentProduct) return [] as string[];
    if (!currentProduct.sizesStock) return sizeOptions;

    return sizeOptions.filter((size) => Number(currentProduct.sizesStock?.[size] ?? 0) > 0);
  ...
  const selectedSizeIsAvailable = availableSizes.includes(selSize);
                      ...
                      {sizeOptions.length ? sizeOptions.map((size) => {
                        ...
                        return (
                          <button
                            key={size}
                            type="button"
                            onClick={() => isAvailable && setSelSize(size)}
                            disabled={!isAvailable}
                            aria-label={`${size}${isAvailable ? '' : ' sin stock'}`}
                          ...
                          >
                            <span className={!isAvailable ? 'line-through' : undefined}>{size}</span>
                          </button>
                        );
                      }) : (
                        <span className="text-sm text-black/50">Sin tallas disponibles</span>
                      )}
                    </div>
                  </div>
