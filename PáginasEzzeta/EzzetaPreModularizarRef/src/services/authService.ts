...
export const buildPermissionsFromRole = (rol: Rol | undefined): Permission[] => {
  ...
  if (groups['Marketing']) {
    const marketingGroup = groups['Marketing'];
    const hasMarketingManage = marketingGroup.accesses.some((access) => {
      const codes = normalizePermissionCodeCollection((access as Record<string, unknown>).permission_codes ?? []);
      return codes.includes('marketing.manage');
    });
    const hasPopUpAccess = marketingGroup.accesses.some((access) => {
      const label = String((access as Record<string, unknown>).label ?? '').trim().toLowerCase();
      const path = String((access as Record<string, unknown>).path ?? '').trim().toLowerCase();
      return label === 'pop-up' || path === '/d-admin/marketing/pop-up';
    });

    if (hasMarketingManage && !hasPopUpAccess) {
      marketingGroup.accesses.push({
        label: 'Pop-Up',
        path: '/D-Admin/marketing/pop-up',
        permission_codes: ['marketing.manage'],
      });
    }
  }
  ...
  if (ventasGroup) {
    ...
    const hasCuponesAnalytics = ventasGroup.accesses.some((access) => {
      const label = String((access as Record<string, unknown>).label ?? '').trim().toLowerCase();
      const path = String((access as Record<string, unknown>).path ?? '').trim().toLowerCase();
      return label === 'cupones' || path === '/d-admin/ventas/cupones';
    });
    ...
    if (hasOrdersManage && !hasCuponesAnalytics) {
      ventasGroup.accesses.push({
        label: 'Cupones',
        path: '/D-Admin/ventas/cupones',
        permission_codes: ['orders.manage'],
      });
    }
...
export const buildFullDashboardPermissions = (): Permission[] => [
  ...
  },
  {
    ...
    accesses: [
      ...
      { label: 'Cupones', path: '/D-Admin/ventas/cupones', permission_code: 'orders.manage' },
    ],
  },
  {
    label: 'Clientes',
  ...
  },
  {
    ...
    accesses: [
      ...
      { label: 'Contacto', path: '/D-Admin/contacto', permission_code: 'contacto.manage' },
